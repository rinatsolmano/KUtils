 ...
 */
@JvmOverloads
fun <T> withTimeMeasurement(timeUnit: TimeUnit = NANOSECONDS, block: () -> T): MeasuredResult<T> {
    ...
    return when (timeUnit) {
        ...
        MILLISECONDS -> MeasuredResult(NANOSECONDS.toMicros(elapsed), result)
 ...
 */
@JvmOverloads
fun measureTime(timeUnit: TimeUnit = NANOSECONDS, block: () -> Unit): Long {
    val start = System.nanoTime()
    block.invoke()
    val elapsed = System.nanoTime() - start
    return when (timeUnit) {
        ...
        MILLISECONDS -> (NANOSECONDS.toMicros(elapsed))
